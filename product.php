...
$db_link = @mysqli_connect($hn,$un,$pw,$db);
					$check="SELECT * FROM `product`where pid='".$_GET['pid']."'";
					...
					$rows=$result->num_rows;
						for($j=0;$j<$rows;$j++)
						{
							...
							<p></p>
							<div class="cart">
							<p><button class="btn btn-primary" style="float:right;" type="submit"  id="cart" name="ok" value="加入購物車">加入購物車</button></p>
							</div>
							</form>
						    </div>';
							}	
							if($_POST['ok']=="加入購物車")
							{
									if($_SESSION['success']=="")
									{
										echo"<script>alert('請登入再下單')</script>";
										echo"<script>window.location.href='signin.php'</script>";
									}	
									else
									{
										$sum=$row['price']*$_POST['cars'];
										$insert="INSERT INTO `book`(`user`,`product`, `quantity`, `images`, `price`, `total`) VALUES ('".$_SESSION['success']."','".$row['product']."',".$_POST['cars'].",'".$row['images']."','".$row['price']."','".$sum."')";
										$result = mysqli_query($db_link, $insert);
										echo"<script>alert('加入成功')</script>";
										echo"<script>window.location.href='product.php?pid=".$row['pid']."'</script>";
									}
							}							
?>
