...
$db_link = @mysqli_connect($hn,$un,$pw,$db);
								...
								echo'<table class="orderss"  align="center" border="1" >
									<tr width="50px">
									<td>訂單編號</td> <td>使用者</td> <td>產品名稱</td> <td>價格</td> <td>數量</td> <td>總額</td> <td>狀態</td>
								    </tr>';
								for($j=0;$j<$rows;$j++)
								{	
									...
									echo'
										<tr>
											<td width="100">'.$row["hid"].'</td> <td width="100">'.$row["user"].'</td> <td width="350">'.$row["product"].'</td> <td width="100">'.$row["price"].'</td> <td width="100">'.$row["quantity"].'</td> <td width="100">'.$sum.'</td> <td width="150">'.$row["situation"].'</td>';
										
									if($row["situation"]=="訂單未完成")
										{
											
											echo'
											<form action="" method="POST">	
											<input type="hidden" name="ban" value='.$row["hid"].'>
											<td><input class=" btn-primary" name="finish"  value="完成訂單" type="submit"></td>
											</tr></form>';
										if($_POST['finish']=="完成訂單")
											{
												$update="UPDATE `history` SET `situation`='訂單完成' WHERE hid='".$_POST['ban']."'";
												$result=mysqli_query($db_link,$update);
										
												echo"<script>alert('訂單完成')</script>";
												echo"<script>window.location.href='mge.php'</script>";
												$_SESSION['hid']="";
											}
										}
